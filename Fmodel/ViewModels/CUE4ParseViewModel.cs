...
public class CUE4ParseViewModel : ViewModel
{
    ...
    public async Task Initialize()
    {
        await _threadWorkerView.Begin(cancellationToken =>
        {
                ...
                case StreamedFileProvider p:
                    switch (p.LiveGame)
                    {
                        case "FortniteLive":
                        {
                            ...
                            {
                                throw new FileLoadException("无法加载最新的堡垒之夜清单,您可能需要切换到本地安装.");
                            }
                            ...
                            {
                                Log.Error("下载清单失败({ManifestUri})", ex.Data["ManifestUri"]?.ToString() ?? "");
                                throw;
                            }
                            ...
                            FLogger.Append(ELog.Information, () =>
                                FLogger.Text($"堡垒之夜[LIVE]已成功加载{elapsedTime.TotalMilliseconds:F1}秒", Constants.WHITE, true));
                            break;
                        }
                        case "ValorantLive":
                        {
                            ...
                            if (manifest == null)
                            {
                                throw new Exception("无法加载最新的无畏契约清单,您可能需要切换到本地安装.");
                            }
                            ...
                            FLogger.Append(ELog.Information, () =>
                                FLogger.Text($"无畏契约'{manifest.Header.GameVersion}'加载成功", Constants.WHITE, true));
                            break;
                        }
                    }
            ...
            Provider.Initialize();
            Log.Information($"{Provider.Versions.Game} ({Provider.Versions.Platform}) |档案: x{Provider.UnloadedVfs.Count} | AES: x{Provider.RequiredKeys.Count}");
        });
    }
    ...
    public void ClearProvider()
    {
        ...
        Helper.CloseWindow<AdonisWindow>("搜索视图");
    ...
    public Task InitMappings(bool force = false)
    {
        ...
        return Task.Run(() =>
        {
            ...
            {
                Log.Information($"映射来自于'{m.FileName}'");
                FLogger.Append(l, () => FLogger.Text($"映射来自于'{m.FileName}'", Constants.WHITE, true));
            }
        });
    }
    ...
    public Task VerifyConsoleVariables()
    {
        if (Provider.Versions["StripAdditiveRefPose"])
        {
            FLogger.Append(ELog.Warning, () =>
                FLogger.Text("附加动画的参考姿势被剥离,这将导致不准确的预览和导出", Constants.WHITE, true));
        }
    ...
    public Task LoadVirtualPaths()
    {
        ...
        return Task.Run(() =>
        {
            ...
            {
                FLogger.Append(ELog.Information, () =>
                    FLogger.Text($"{_virtualPathCount}加载虚拟路径", Constants.WHITE, true));
            }
            else
            {
                FLogger.Append(ELog.Warning, () =>
                    FLogger.Text("无法加载虚拟路径,插件清单可能不存在", Constants.WHITE, true));
            }
        });
    }
    ...
    public void Extract(CancellationToken cancellationToken, GameFile entry, bool addNewTab = false, EBulkType bulk = EBulkType.None)
    {
        ...
        switch (entry.Extension)
        {
            ...
            case "ttf":
                FLogger.Append(ELog.Warning, () =>
                    FLogger.Text($"导出{entry.Name}原始数据并更改其扩展名,如果您希望它是一个可安装的字体文件", Constants.WHITE, true));
                break;
            ...
            {
                FLogger.Append(ELog.Warning, () =>
                    FLogger.Text($"包'{entry.Name}'的类型未知.", Constants.WHITE, true));
                break;
            }
        }
    }
    ...
    {
        Log.Information("User CTRL-CLICKED提取'{完整路径}'", fullPath);
        ...
        switch (dummy)
        {
            ...
            case USvgAsset when (isNone || saveTextures) && pointer.Object.Value is USvgAsset svgasset:
            {
                ...
                if (saveTextures)
                {
                    ...
                    if (File.Exists(path))
                    {
                        Log.Information("{文件名}成功保存", fileName);
                        if (updateUi)
                        {
                            FLogger.Append(ELog.Information, () =>
                            {
            ...
            case USoundWave when isNone:
            {
                ...
                if (data == null || !hasAf)
                {
                    if (hasAf) FLogger.Append(ELog.Warning, () => FLogger.Text($"不支持的音频格式'{audioFormat}'", Constants.WHITE, true));
                    return false;
                }
    ...
    private void SaveExport(UObject export, bool updateUi = true)
    {
        ...
        {
            Log.Information("成功保存{文件路径}", savedFilePath);
            if (updateUi)
            {
                FLogger.Append(ELog.Information, () =>
                {
        ...
        {
            Log.Error("{文件名}无法保存", export.Name);
            FLogger.Append(ELog.Error, () => FLogger.Text($"无法保存'{export.Name}'", Constants.WHITE, true));
        }
    }
    ...
    public void ExportData(GameFile entry, bool updateUi = true)
    {
        if (Provider.TrySavePackage(entry, out var assets))
        {
            ...
            Log.Information("{文件名}成功导出", entry.Name);
            if (updateUi)
            {
                FLogger.Append(ELog.Information, () =>
                {
                    FLogger.Text("成功导出 ", Constants.WHITE);
        ...
        {
            Log.Error("{文件名}无法导出", entry.Name);
            if (updateUi)
                FLogger.Append(ELog.Error, () => FLogger.Text($"无法导出'{entry.Name}'", Constants.WHITE, true));
        }
    }
