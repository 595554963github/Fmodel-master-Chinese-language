...
public class UsmapParser
{
    ...
    public UsmapParser(FArchive archive)
    {
        ...
        if (magic != FileMagic)
            throw new ParserException("Usmap的魔数无效");

        Version = archive.Read<EUsmapVersion>();
        if (Version > EUsmapVersion.Latest)
            throw new ParserException($"Usmap的版本无效 ({(byte) Version})");
        ...
        switch (CompressionMethod)
        {
            case EUsmapCompressionMethod.None:
            {
                if (compSize != decompSize)
                    throw new ParserException("未压缩:压缩尺寸必须等于解压尺寸");
            ...
            default:
                throw new ParserException($"无效的压缩方法{CompressionMethod}");
        }
