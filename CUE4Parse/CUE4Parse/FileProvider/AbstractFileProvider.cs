    ...
    public abstract class AbstractFileProvider : IFileProvider
    {
        ...
        public GameFile this[string path]
            => TryGetGameFile(path, Files, out var file)
                ? file
                : throw new KeyNotFoundException($"该路径下不存在游戏文件\"{path}\"");
        ...
        {
            if (!file.IsUePackage) throw new ArgumentException("无法加载非UE包", nameof(file));
            ...
            switch (file)
            {
                ...
                default:
                    throw new NotImplementedException($"{file.GetType()}不是支持的类型");
            }
        }
