...
namespace CUE4Parse.UE4.Pak
{
    public partial class PakFileReader : AbstractAesVfsReader
    {
        ...
        public PakFileReader(FArchive Ar) : base(Ar.Name, Ar.Versions)
        {
            ...
            {
                Log.Warning($"Pak文件\"{Name}\"有不受支持的版本{(int) Info.Version}");
            }
        }
        ...
        {
            if (entry is not FPakEntry pakEntry || entry.Vfs != this) throw new ArgumentException($"错误的pak文件读取,必须为{entry.Vfs.Name},这是{Name}");
        ...
        private void ReadIndexLegacy(StringComparer pathComparer)
        {
            ...
            {
                throw new InvalidAesKeyException($"给定的aes密钥'{AesKey?.KeyString}'不起作用'{Name}'", e);
            }
        ...
        private void ReadIndexUpdated(StringComparer pathComparer)
        {
            ...
            if (!primaryIndex.ReadBoolean())
                throw new ParserException(primaryIndex, "无路径哈希索引");
            ...
            if (primaryIndex.Read<int>() < 0)
                throw new ParserException("检测到损坏的pak主要索引");

            // Read FDirectoryIndex
            Ar.Position = directoryIndexOffset;
            var directoryIndex = new FByteArchive($"{Name} - 目录索引", ReadAndDecrypt((int) directoryIndexSize));
            if (Ar.Game == EGame.GAME_Rennsport)
            {
                Ar.Position = directoryIndexOffset;
                directoryIndex = new FByteArchive($"{Name} - 目录索引",
