    ...
    public abstract class FArchive : RandomAccessStream, ICloneable
    {
        ...
        public T[] ReadBulkArray<T>(int elementSize, int elementCount, Func<T> getter)
        {
            ...
            if (Game != EGame.GAME_HogwartsLegacy && Position != pos + array.Length * elementSize)
                throw new ParserException($"RawArray项目大小不匹配:预期{elementSize}, 序列化{(Position - pos) / array.Length}");
            return array;
        }
        ...
        public bool ReadBoolean()
        {
            ...
            return i switch
            {
                0 => false,
                1 => true,
                _ => throw new ParserException(this, $"无效的bool值({i})")
            };
        }
        ...
        public virtual string ReadFString()
        {
            ...
            if (length == int.MinValue)
                throw new ArgumentOutOfRangeException(nameof(length), "档案已损坏");

            if (Math.Abs(length) > Length - Position)
            {
                throw new ParserException($"无效的FString长度'{length}'");
            }
            ...
            if (length < 0) // LoadUCS2Char, Unicode, 16-bit, fixed-width
            {
                unsafe
                {
                    ...
                    fixed (byte* ucs2BytesPtr = ucs2Bytes)
                    {
                        ...
                        {
                            throw new ParserException(this, "序列化FString不是null终止的");
                        }
#endif
        ...
        {
            throw new InvalidOperationException("Generic FArchive无法读取UObject's");
        }
        ...
        public void SerializeCompressedNew(byte[] dest, int length, string compressionFormatToDecodeOldV1Files, ECompressionFlags flags, bool bTreatBufferAsFileReader, out long outPartialReadLength)
        {
            ...
            {
                throw new ParserException(this, "BulkData压缩标头读取错误,此包可能已损坏!");
            }
            ...
            if (!bReadCompressionFormat)
            {
                ...
                {
                    Log.Warning("旧式压缩标志正在与FAsyncCompressionChunk一起使用,请使用此更新任何代码!");
            ...
            {
                throw new ParserException(this, "BulkData压缩标头读取错误,此包可能已损坏!\n要解码的压缩格式未找到: " + compressionFormatToDecode);
            }
            ...
            if (summary.UncompressedSize > length) throw new ParserException(this, $"存档序列化压缩/未压缩大小({summary.UncompressedSize}) >长度({length})");
            ...
            if (totalChunkCompressedSize != summary.CompressedSize) throw new ParserException(this, $"存档序列化压缩总块压缩大小({totalChunkCompressedSize}) != Summary.CompressedSize ({summary.CompressedSize})");
            if (totalChunkUncompressedSize != summary.UncompressedSize) throw new ParserException(this, $"存档序列化压缩的总块未压缩大小({totalChunkUncompressedSize}) != Summary.UnompressedSize ({summary.UncompressedSize})");
            ...
            for (var chunkIndex = 0; chunkIndex < totalChunkCount; chunkIndex++)
            {
                ...
                {
                    throw new ParserException(this, $"无法解压缩数据{Name},要解码的压缩格式={compressionFormatToDecode}", e);
                }
        ...
        public void CheckReadSize(int length)
        {
            if (length < 0)
            {
                throw new ParserException(this, "读取大小小于零.");
            }
            if (Position + length > Length)
            {
                throw new ParserException(this, "读取大小大于剩余档案长度.");
            }
        }
