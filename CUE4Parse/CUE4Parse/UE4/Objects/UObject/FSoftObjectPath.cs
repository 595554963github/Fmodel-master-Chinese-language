    ...
    public readonly struct FSoftObjectPath : IUStruct
    {
        ...
        public FSoftObjectPath(FAssetArchive Ar)
        {
            if (Ar.Ver < EUnrealEngineObjectUE4Version.ADDED_SOFT_OBJECT_PATH)
            {
                ...
                throw new ParserException(Ar, $"资源路径\"{path}\"是简短的形式,不支持,也不推荐");
            }
        ...
        public UExport Load() =>
            Load(Owner?.Provider ?? throw new ParserException("包在没有IFileProvider的情况下加载"));
        ...
        public T Load<T>() where T : UExport =>
            Load<T>(Owner?.Provider ?? throw new ParserException("包在没有IFileProvider的情况下加载"));
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public async Task<UExport> LoadAsync() => await LoadAsync(Owner?.Provider ?? throw new ParserException("包在没有IFileProvider的情况下加载"));
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public async Task<T> LoadAsync<T>() where T : UExport => await LoadAsync<T>(Owner?.Provider ?? throw new ParserException("包在没有IFileProvider的情况下加载"));
        ...
        public T Load<T>(IFileProvider provider) where T : UExport =>
            Load(provider) as T ?? throw new ParserException("加载了软对象属性,但类型错误");
        ...
        public async Task<T> LoadAsync<T>(IFileProvider provider) where T : UExport => await LoadAsync(provider) as T ??
                                                                                       throw new ParserException("加载了软对象属性,但类型错误");
