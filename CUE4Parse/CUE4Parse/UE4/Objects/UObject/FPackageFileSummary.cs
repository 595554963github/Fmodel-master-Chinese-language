...
namespace CUE4Parse.UE4.Objects.UObject
{
    ...
    public class FPackageFileSummary
    {
        ...
        internal FPackageFileSummary(FArchive Ar)
        {
            ...
            {
                throw new ParserException($"无效的uasset魔数: 0x{Tag:X8} != 0x{PACKAGE_FILE_TAG:X8}");
            }
            ...
            if (Tag == PACKAGE_FILE_TAG_SWAPPED)
            {
                ...
                throw new ParserException("包的字节不支持交换");
            }
            ...
            if (legacyFileVersion < 0) // means we have modern version numbers
            {
                if (legacyFileVersion < CurrentLegacyFileVersion)
                {
                    ...
                    throw new ParserException("无法加载旧版UE3文件");
                }
            ...
            {
                throw new ParserException($"无效的压缩标志({(uint) CompressionFlags})");
            }
            ...
            {
                throw new ParserException("启用包级压缩");
            }
