        ...
        public static void Decompress(byte[] compressed, int compressedOffset, int compressedSize, byte[] uncompressed, int uncompressedOffset, int uncompressedSize, CompressionMethod method, FArchive? reader = null)
        {
            ...
            switch (method)
            {
                ...
                case CompressionMethod.LZ4:
                    ...
                    if (result != uncompressedSize) throw new FileLoadException($"解压LZ4数据失败(预期: {uncompressedSize}, 结果: {result})");
                    return;
                ...
                default:
                    if (reader != null) throw new UnknownCompressionMethodException(reader, $"压缩法\"{method}\"是未知的");
                    else throw new UnknownCompressionMethodException($"压缩法\"{method}\"是未知的");
            }
        }
    }
}
