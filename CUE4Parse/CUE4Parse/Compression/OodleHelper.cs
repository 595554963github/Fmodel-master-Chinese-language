...
public static class OodleHelper
{
        ...
        byte[] uncompressed, int uncompressedOffset, int uncompressedSize, FArchive? reader = null)
    {
        if (Instance is null)
        {
            const string message = "Oodle解压失败:未初始化";
        ...
        if (decodedSize <= 0)
        {
            var message = $"oodle解压失败,结果{decodedSize}";
            if (reader is not null) throw new OodleException(reader, message);
            throw new OodleException(message);
        }
    ...
    public static async Task<bool> DownloadOodleDllAsync(string? path)
    {
        ...
        try
        {
            ...
            if (dllUrl == null)
            {
                Log.Warning("Warframe索引不包含oodle dll");
                return false;
            }
            ...
            Log.Information($"成功下载oodle.dll在\"{path}\"");
            return true;
        }
        catch (Exception e)
        {
            Log.Warning(e, "下载oodle.dll时未捕获异常");
        }

        Log.Information("从其他来源下载oodle.dll");
    ...
    public static async Task<bool> DownloadOodleDllFromOodleUEAsync(HttpClient client, string path)
    {
        ...
        try
        {
            ...
            ArgumentNullException.ThrowIfNull(entry, "未找到zip中的oodle条目");
        ...
        {
            Log.Warning(e, "从OodleUE下载oodle.dll时未捕获异常");
        }
        return false;
    }
}
