...
public static class TextureDecoder
{
        ...
        if (texture.PlatformData is { FirstMipToSerialize: >= 0, VTData: { } vt } && vt.IsInitialized())
        {
            ...
            for (uint layer = 0; layer < vt.NumLayers; layer++)
            {
                ...
                for (uint tileIndexInMip = 0; tileIndexInMip < tileOffsetData.MaxAddress; tileIndexInMip++)
                {
                    ...
                    {
                        throw new NotSupportedException("不支持单个虚拟映像中的多个像素格式/颜色类型");
                    }
    ...
    {
        if (mip?.BulkData.Data is not { Length: > 0 }) throw new ParserException("提供的MipMap为空或数据为空!");
        if (PixelFormatUtils.PixelFormats.ElementAtOrDefault((int) format) is not { Supported: true } formatInfo || formatInfo.BlockBytes == 0) throw new NotImplementedException($"{format}是不支持提供的像素格式 !");
        ...
        if (isXBPS || isNX)
        {
            ...
            if (blockSizeX * blockSizeY > totalBlocks) throw new ParserException("提供的MipMap无法取消平铺!");
        }
    ...
    private static void DecodeBytes(byte[] bytes, int sizeX, int sizeY, int sizeZ, FPixelFormatInfo formatInfo, bool isNormalMap, out byte[] data, out SKColorType colorType)
    {
        switch (formatInfo.UnrealFormat)
        {
            ...
            case EPixelFormat.PF_A32B32G32R32F:
                ...
                colorType = SKColorType.Rgba8888;
                break;
            default: throw new NotImplementedException($"未知像素格式:{formatInfo.UnrealFormat}");
        }
    }
