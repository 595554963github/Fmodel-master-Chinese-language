...
public static class SoundDecoder
{
    public static void Decode(this UObject export, bool shouldDecompress, out string audioFormat, out byte[]? data)
    {
        switch (export)
        {
            case UAkMediaAssetData mediaAsset:
                mediaAsset.Decode(shouldDecompress, out audioFormat, out data);
                break;
            case USoundWave soundWave:
                soundWave.Decode(shouldDecompress, out audioFormat, out data);
                break;
            default:
    ...
    public static void Decode(this USoundWave soundWave, bool shouldDecompress, out string audioFormat, out byte[]? data)
    {
        ...
        else if (soundWave.RunningPlatformData != null && soundWave.RunningPlatformData.Chunks != null)
        {
            var offset = 0;
            ...
            {
                if (soundWave.RunningPlatformData.Chunks[i].BulkData != null && soundWave.RunningPlatformData.Chunks[i].BulkData.Data != null)
                {
    ...
    public static void Decode(this UAkMediaAssetData mediaData, bool shouldDecompress, out string audioFormat, out byte[]? data)
    {
        ...
        {
            if (dataChunk.IsPrefetch)
                continue;
            if (dataChunk.Data != null && dataChunk.Data.Data != null)
            {
                lengthSum += dataChunk.Data.Data.Length;
            }
        }
        var input = new byte[lengthSum];
