    ...
    public static class AnimConverter
    {
        ...
        private static CAnimSequence ConvertSequence(this UAnimSequence animSequence, USkeleton skeleton)
        {
            ...
            else switch (animSequence.CompressedDataStructure)
            {
                ...
                default:
                    throw new ArgumentOutOfRangeException("不支持的压缩数据类型" + animSequence.CompressedDataStructure?.GetType().Name);
            }
        ...
        private static void ReadPerTrackQuatData(FArchive Ar, string trackKind, ref FQuat[] dstKeys, ref float[] dstTimeKeys, int numFrames)
        {
            ...
            for (var keyIndex = 0; keyIndex < numKeys; keyIndex++)
            {
                dstKeys[keyIndex] = keyFormat switch
                {
                    ...
                    _ => throw new ParserException(Ar, $"未知{trackKind}压缩法: {(int) keyFormat} ({keyFormat})")
                };
            }
            // align to 4 bytes
            Ar.Position = Ar.Position.Align(4);
            ...
            for (var keyIndex = 0; keyIndex < numKeys; keyIndex++)
            {
                switch (keyFormat)
                {
                    ...
                    case ACF_Identity:
                        ...
                        break;
                    default:
                        throw new ParserException(Ar, $"未知{trackKind}压缩法: {(int) keyFormat} ({keyFormat})");
                }
            }
            // align to 4 bytes
            Ar.Position = Ar.Position.Align(4);
        ...
        private static void ReadKeyLerpData(FArchive reader, UAnimSequence animSequence, CAnimTrack track, int trackIndex, bool hasTimeTracks)
        {
            ...
            void ReadVectorKeys(long offset, int numKeys, AnimationCompressionFormat compressionFormat, FVector[] keys)
            {
                reader.Position = offset;
                if (numKeys == 1) compressionFormat = ACF_None; // single key is stored without compression

                // read mins/ranges
                FVector mins = FVector.ZeroVector;
                FVector ranges = FVector.ZeroVector;
                if (compressionFormat == ACF_IntervalFixed32NoW)
                {
                ...
                for (var keyIndex = 0; keyIndex < numKeys; keyIndex++)
                {
                    keys[keyIndex] = compressionFormat switch
                    {
                        ...
                        _ => throw new ParserException($"未知向量密钥压缩方法: {(int) compressionFormat} ({compressionFormat})")
                    };
                }
            }
            ...
            if (transKeys > 0)
            {
                ReadVectorKeys(transOffset, transKeys, compressedData.TranslationCompressionFormat, track.KeyPos);
            ...
            {
                ReadVectorKeys(scaleOffset, scaleKeys, compressedData.ScaleCompressionFormat, track.KeyScale);
            ...
            {
                reader.Position = rotOffset;
                var compressionFormat = compressedData.RotationCompressionFormat;
                if (rotKeys == 1) compressionFormat = ACF_Float96NoW;
                ...
                {
                    track.KeyQuat[k] = compressionFormat switch
                    {
                        ...
                        ACF_Identity => FQuat.Identity,
                        _ => throw new ParserException($"未知旋转压缩法: {(int) compressionFormat} ({compressionFormat})")
                    };
                }
